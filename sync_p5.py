#!/usr/bin/env python3
"""Part 5/5: 3520~end, 连218.75.126.9:7709"""
...
part = codes[3520:]
print(f'part5: {len(part)}只, {part[0]}~{part[-1]}', flush=True)
client = Quotes.factory(market='std', tcp=('218.75.126.9', 7709, True))
...
for j, code in enumerate(part):
    try:
        ...
        try: client = Quotes.factory(market='std', tcp=('218.75.126.9', 7709, True))
        except: pass
    if (j+1) % 50 == 0 or j+1 == len(part):
        conn.commit()
        print(f'[part5] {j+1}/{len(part)}', flush=True)
conn.close()
print(f'[part5] done!', flush=True)
