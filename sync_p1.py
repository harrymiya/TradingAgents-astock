#!/usr/bin/env python3
"""Part 1/5: 000001开始, 连202.108.253.139:80"""
...
part = codes[:880]
print(f'part1: {len(part)}只, {part[0]}~{part[-1]}', flush=True)
...
for j, code in enumerate(part):
    try:
        ...
        try: client = Quotes.factory(market='std', tcp=('202.108.253.139', 80, True))
        except: pass
    if (j+1) % 50 == 0 or j+1 == len(part):
        conn.commit()
        print(f'[part1] {j+1}/{len(part)}', flush=True)

conn.close()
print(f'[part1] done! {len(part)}只', flush=True)
