#!/usr/bin/env python3
"""Part 4/5: 2640~3520, 连115.238.56.198:7709"""
...
part = codes[2640:3520]
print(f'part4: {len(part)}只, {part[0]}~{part[-1]}', flush=True)
client = Quotes.factory(market='std', tcp=('115.238.56.198', 7709, True))
...
for j, code in enumerate(part):
    try:
        ...
        try: client = Quotes.factory(market='std', tcp=('115.238.56.198', 7709, True))
        except: pass
    if (j+1) % 50 == 0 or j+1 == len(part):
        conn.commit()
        print(f'[part4] {j+1}/{len(part)}', flush=True)
conn.close()
print(f'[part4] done!', flush=True)
